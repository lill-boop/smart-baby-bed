...
// 当App控制音乐时�置�1, main.c将不再根据voice�动控�
// 当�为0�, 恢�自动模�(voice�测控�)
...
void Ali_MQTT_Recevie(void)
{
    ...
    // music_flag=1 从App表示"��", �要lullabuy(0)拉低触发
    // music_flag=0 从App表示"停�", �要lullabuy(1)拉高待机
    // music_flag=-1 从App表示"恢�自动模�"
    ...
    if(Start != NULL)
    {
      ...
      if(Start != NULL) {
          ...
          if(val == -1) {
              // 恢�自动模�: App不再覆盖, main.c根据voice�动控�
              music_app_override = 0;
              music_flag = 0;
              uwifi_printf("LOG: CMD HIT: music_flag=-1, restored AUTO mode\r\n");
          } else {
              // App手动控制: 设置覆盖标志
              music_app_override = 1;  // 阻�main.c�动控�
              music_flag = (val != 0) ? 1 : 0;
              // App的music_flag=1表示��, �要传0给lullabuy(拉低触发)
              lullabuy(!music_flag);  // 反转: music_flag=1→lullabuy(0)→LOW→播�
