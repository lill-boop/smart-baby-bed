...
// DY-SV17F 语音模块控制 (IO触发模式)
// lullabuy(0) = 拉低GPIO = 触发播放 (当检测到哭声时调用)
// lullabuy(1) = 拉高GPIO = 停止/待机 (当无哭声时调用)
...
// 从 AliESP8266.c 导入: App音乐控制覆盖标志
// 当 music_app_override==1 时, main.c不自动根据voice控制音乐
extern uint8_t music_app_override;
